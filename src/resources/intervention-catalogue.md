...
Most organizations reach for the same tools when they feel friction: more process, more training, more structure. These interventions are real, but they are also expensive — in time, attention, and energy.

A minimally invasive intervention targets the structural root of a problem while adding the least possible weight. It respects the fact that your team is already busy, and that every new framework has a hidden cost.

The principle is simple: _subtract before you add._

### Second-order intervention

A first-order intervention changes what you do. A second-order intervention changes how you do it. When a standard intervention feels too heavy, the question is not "what more can we do?" but "can we solve this through a different component entirely?"

### Three comparisons

**Problem: Team lacks a shared understanding of priorities.**

The standard approach is to create a new priority-setting process — weekly reviews, scoring frameworks, alignment meetings. The minimally invasive alternative is to write a one-page priority document and pin it where the team already communicates. Artifact instead of process. No new meeting required.

**Problem: Too many meetings, people feel drained.**

The standard approach is to redesign the meeting policy — strict agendas, time limits, mandatory preparation. The minimally invasive alternative is to remove one recurring meeting from everyone's calendar. Policy subtraction instead of process addition. The problem often solves itself.

**Problem: A new tool isn't being adopted.**

The standard approach is to train the entire team in a half-day workshop. The minimally invasive alternative is to integrate the tool into one existing workflow so it shows up where people already work. Technology integration instead of people training. Adoption follows from convenience, not mandate.
